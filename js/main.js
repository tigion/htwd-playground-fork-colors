...
function handleRandomColor(e) {
    ...
    const min = 50;  // 0
    const max = 255; // 255
    document.getElementById("r").value = colorValue(Math.floor(Math.random() * (max - min)) + min);
    document.getElementById("g").value = colorValue(Math.floor(Math.random() * (max - min)) + min);
    document.getElementById("b").value = colorValue(Math.floor(Math.random() * (max - min)) + min);
    updateColorPreview();
}
...
function recordAsHTML(id, r, g, b) {
    ...
    return '<span title="' + title + '" style="background-color: ' + color + '"></span>';
}
